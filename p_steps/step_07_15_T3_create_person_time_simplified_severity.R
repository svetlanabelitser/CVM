...
for (subpop in subpopulations_non_empty) {
  ...
  start_week <- seq.Date(as.Date("19000101","%Y%m%d"), study_end, by = "week")
  start_week <- start_week[start_week >= study_start]
  ...
  for (i in 1:length(start_week)){
    ...
    assign(nameoutput,CountPersonTime(
      ...
      Strata = c("sex","Dose","type_vax", "any_risk_factors"),
  ...
  fwrite(persontime_benefit_week,file=paste0(thisdirexp,"D4_persontime_severity_week.csv"))
  
  nameoutput<-paste0("D4_persontime_severity_week",suffix[[subpop]])
  assign(nameoutput,persontime_benefit_week)
  save(nameoutput,file=paste0(diroutput,nameoutput,".RData"),list=nameoutput)
  
  rm(list=paste0("D4_persontime_severity_week",suffix[[subpop]]))
...
for (subpop in subpopulations_non_empty){
  tempname<-paste0("D4_persontime_severity_week")
  thisdirexp <- ifelse(this_datasource_has_subpopulations == FALSE,direxp,direxpsubpop[[subpop]])
  ...
  suppressWarnings(
    DRE_Treshold(
      ...
      FileContains = "D4_persontime_severity_week"
    )
  )
  rm(list=tempname)
}
