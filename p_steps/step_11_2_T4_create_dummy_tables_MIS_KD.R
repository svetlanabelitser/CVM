...
create_empty_table_1a <- function(x) {
  n6 <- numeric(6)
  row_names_1 <- c("Start population", "A",
                   "D_death_before_study_entry", "E_no_observation_period_including_study_start",
                   "F_age_over18", "end population")
  ext = data.table(a = row_names_1, datasource = n6)
  setnames(ext, "datasource", x)
  return(ext)
}

create_empty_table_1b <- function(x) {
  n2 <- numeric(2)
  row_names_2 <- c("Less than 365 days history at 1/1/2020", "Study population")
  ext = data.table(a = row_names_2, datasource = n2)
  setnames(ext, "datasource", x)
  return(ext)
}
##-------------------------
...
if ("datasource" %not in% names(flow_source)) {
  ...
  setnames(flow_study, "datasource", "Datasource")
}
...
flow_source <- data.table::melt(flow_source, id.vars = c("row_id", "Datasource", "N"),
                                measure.vars = c("A_sex_or_birth_date_missing", "B_birth_date_absurd",
                                                 ...
                                                 "E_no_observation_period_including_study_start",
                                                 "F_age_over18"), variable.name = "a")
...
empty_table_1 <- create_empty_table_1a(export_dap_name)
...
empty_table_2 <- create_empty_table_1b(export_dap_name)
...
table_1b <- table_1b[a == "F_age_over18", a := "Over 18 years old at study entry"]
table_1b <- table_1b[a == "end population", a := "Persons in the data source at study start"]
setnames(table_1a, "a", " ")
setnames(table_1b, "a", " ")


fwrite(table_1b, file = paste0(dummytables, "Attrition diagram 1.csv"))
...
daps <- intersect(export_dap_name, names(table2))
...
fwrite(table3_4_5_6, file = paste0(dummytables,
...
col_to_keep <- intersect(c("a", "Parameters", export_dap_name), names(total_pop_c))
...
daps <- intersect(export_dap_name, names(table5))
daps_perc <- paste("perc", daps, sep="_")
col_order <- c(rbind(daps, daps_perc))
table5 <- table5[, (daps_perc) := character(nrow(table5))]
total_pop_c <- total_pop_c[, ..daps]
pt_total_c <- pt_total_c[, ..daps]
table5 <- table5[a %in% c("Age in categories", "Month of first diagnosis", "At risk population at first covid diagnosis"),
...
table5 <- table5[a %in% c("Age in categories", "Person years across sex", "At risk population at first covid diagnosis", 
                          "Person years across age categories"), (daps_perc) := lapply(.SD, paste0, "%"), .SDcols = daps_perc]
...
fwrite(table5, file = paste0(dummytables, "Cohort characteristics at first occurrence of COVID-19 prior to vaccination (cohort c).csv"))
...
fwrite(table_7, file = paste0(dummytables, "COVID-19 vaccination by dose and time period between first and second dose (days).csv"))
...
# 
# fwrite(table_7, file = paste0(dummytables, "Code counts for narrow definitions (for each event) separately.csv"))
...
# 
# fwrite(table_12, file = paste0(dummytables, "Incidence of each concept (narrow) per 100,000 PY prior to vaccination and COVID-19.csv"))
# 
# 
# 
# 
...
#          c("Age in 2020", "Sex", "Person years", "IR narrow", "LL narrow", "UL narrow"))
# 
# fwrite(table_12, file = paste0(dummytables, "Incidence of each concept (narrow) per 100,000 PY after COVID-19 and prior to vaccination.csv"))
# 
# 
# 
# 
...
#            c("Age in 2020", "Sex", "Person years", "IR narrow", "LL narrow", "UL narrow"))
#   
#   fwrite(table_12, file = paste0(dummytables, "Incidence of each concept (narrow) per 100,000 PY after vaccination (",
#                                  vax_m,").csv"))
# }
