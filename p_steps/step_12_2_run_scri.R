...
for (subpop in subpopulations_non_empty) {
  ...
  # 'data_vax' - dataset with one row per vax per id (or a missing value if no vax); 
  #               AND with all events variables 
  #               AND with variable 'covid19_date' (? "covid")
  # 
  
  #  if events are NOT in 'data_vax' ==> merge 'data_vax' and 'data_events'   
  
  
  
  
  
  dap <- ifelse( any(names(data_vax)=="DAP"), data_vax$DAP[1], "")
  ...
  id            <- "person_id"
  start_obs     <- "study_entry_days"
  end_obs       <- "study_exit_days"
  censored_vars <- "death_days"
  
  
  
  #  vax             :"type_vax"  "vax_days", "vax_date"   OR  "days_vax", "date_vax"  
  
  #  other variables :  sex, age
  
  #   events          : "myocarditis","myocarditis_days" ,  "myocarditis_date", ...  
  
  #  ae_events <-  c("myocarditis","pericarditis"  )   <== check that they are all events!!! 
  ...
  # outcome variables:
  ae_events <-  c("myocarditis","pericarditis"  )  # c("myocarditis","pericarditis", ...)
  ...
  for(i in 1:length(lengths))
    time_seq[[i]] <- seq(min(data_vax[,start_obs],na.rm=T)-starts[i],max(data_vax[,end_obs],na.rm=T)+lengths[i]-1,by=lengths[i])
  ...
  for(iae in ae_events){ 
    ...
    print(table1( unique(data_vax[ ,c("person_id", iae)]) [ , iae]  ))
  }
  ...
  if(lmain){
    ...
    while(T){
      cond_vax_i <- data_vax$vax_n == ivax
      cond2<-duplicated(data_vax$id[cond_vax_i])
      if(!any(cond2)) break
      ivax <- ivax + 1
    ...
    #### 'type_vax_short'
    data_vax$type_vax_short <- substring(data_vax$type_vax,1,5)
    ...
    #
    extra_options <- list( paral                = F,    # if T ==> library(parallel) is started in function 'scri'
    ...
    for(iae in ae_events){
      ...
      #
      vax_def0 <- scri_data_parameters(data=data_vax, vax_name="vax_number", vax_time="vax_days", vax_date="date_vax" )
      ...
      vax_def <- define_rws(vax_def0,  cut_points_before = c(-90,-29,0), cut_points_after = c(0,1,29,62,182), cut_points_name="28d", no_last_interval_after=T, 
                            data=data_vax, vax_dep = c( before="type_vax_short" ))
      ...
      data_vax$age30     <- paste0("age",as.character(cut(data_vax$age_at_study_entry, c(-1,30,   Inf))))
      data_vax$age30_50  <- paste0("age",as.character(cut(data_vax$age_at_study_entry, c(-1,30,50,Inf))))
      data_vax$sexc      <- paste0("sex:",data_vax$sex )
      data_vax$sex_age30 <- paste0("sex:",data_vax$sex, " ", data_vax$age30)
      ...
      for( strata_var in c( "age30","age30_50", "sexc", "sex_age30") ){
        ...
        strata_values <- unique(data_vax[,strata_var])
      ...
      #            for covid subsets
      #
      if(any(tolower(names(data_vax0)) != "covid19_date") ){
        ...
        cat("\n\nALL DATA: table for variables:\n")
        print(substring(iae,1,7))
        table1( unique(data_vax[ ,c("person_id", iae)]) [ , iae]  )
        ...
        for(icovid in c( "covidplus30d","nocovid")){
          ...
          if(icovid == "covidplus30d"){
            ...
            data_vax[,"no_covid_before_event_plus30d"] <- ( data_vax[,paste0(iae,"_date")] + 30 ) < data_vax$covid19_date  &  !is.na(data_vax$covid19_date) & !is.na(data_vax[,paste0(iae,"_date")])
            data_vax[is.na(data_vax$covid19_date),"no_covid_before_event_plus30d"] <- T
            ...
            print(table1( unique( data_vax[ ,c("person_id", "covid","covid_selection_name")] )[ ,c("covid","covid_selection_name")] ))
          }
          ...
          if(icovid =="nocovid"){
            ...
            data_vax[                                          , "covid_selection_name"] <- ""
            data_vax[data_vax[,"no_covid_before_event_plus30d"], "covid_selection_name"] <- "no_covid_observed"
  ...
  #########################################################################
  #
  #  this part is only for "myocarditis"  &  {'no_split', 'brand'}  &  without time adjustment
  #
  if(ldist){
    ...
    data_vax$dist[-1][ data_vax$id[-1]!=data_vax$id[-nrow(data_vax)]  ] <- NA
    #### 'dist_gt_60'
    ...
    data_vax$type_with_prev <- format(data_vax$type_vax_short)
    ...
    if(any(cond_prev_exists)) 
      data_vax$type_with_prev[ cond_prev_exists ] <- paste0( data_vax$type_with_prev[cond_next_exists],"-", format(data_vax$type_vax_short)[cond_prev_exists] )
    
    # create variable with history of brands:
    data_vax$type_history <- format(data_vax$type_vax_short)
    prev_steps <- 1
    while(T){
      ...
      data_vax$type_history[ cond_prev_exists ] <- paste0( data_vax$type_history[cond_next_exists],"-", format(data_vax$type_vax_short)[cond_prev_exists] )
      prev_steps <- prev_steps + 1
    }
    # table1(data_vax[,,"type_history"])
    
    # create variable with sorted history of brands:
    data_vax$type_history_sorted <- lapply( strsplit(data_vax$type_history, "[ |-]+"), function(x)paste0(sort(x), collapse ="-") )
    ...
    for(iae in "myocarditis" ){
      ...
      ## cut_points_name="28d" :  { [-91;-30], [-29;-1], [0;0], [1;28], [28;61]  } 
      vax_def <- define_rws(vax_def0,  cut_points_before = c(-90,-29,0), cut_points_after = c(0,1,29,62), cut_points_name="28d", no_last_interval_after=T, 
                            data=data_vax, vax_dep = c( before="type_vax_short", after="dist_gt_60"  ))
      res <- scri( formula = "~ brand:lab", vax_def = vax_def, data = data_vax, event_info=event_info, extra_parameters = extra_options, add_to_itself=F )
      ...
      #
      vax_def0 <- scri_data_parameters(data=data_vax, vax_name="vax_name", vax_time="vax_days", vax_date="date_vax" )
      ...
      # without formula:
      res <- try( scri( vax_def = vax_def, data = data_vax, event_info=event_info, extra_parameters = extra_options, add_to_itself=F ) )
