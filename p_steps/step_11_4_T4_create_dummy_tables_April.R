...
create_empty_table_1a <- function() {
  n6 <- numeric(5)
  row_names_1 <- c("Start population", "A",
...
create_empty_table_1b <- function() {
  n4 <- numeric(2)
  row_names_2 <- c("Less than 365 days history at any point in time after 1.1.2019 
                           (and not born after 1.1.2019), N", "Final study population, N")
  if (length(intersect(names(flow_source_1a), c("Italy_ARS", "NL_PHARMO", "UK_CPRD", "ES_BIFAP"))) == 1) {
...
for (subpop in subpopulations_non_empty) {
  ...
  if(this_datasource_has_subpopulations == T)   dummytables_october <- paste0(thisdirexp,"Dummy tables October/")
  ...
  correct_row_names <- empty_table_1$a[2:nrow(flow_source_1a)]
  ...
  flow_study <- flow_study[value == 0, a := "Less than 365 days history at any point in time after 1.1.2019 
                           (and not born after 1.1.2019), N"]
  flow_study <- flow_study[value == 1, a := "Final study population, N"][, .(Datasource, N, a)]
  flow_study <- data.table::dcast(flow_study, a ~ Datasource, value.var = c("N"))
  empty_table_2 <- create_empty_table_1b()
  flow_source_1b <- rbind(empty_table_2, flow_study, fill = T)
  ...
  diagnosis_concepts <- names(concept_set_domains[concept_set_domains == "Diagnosis"])
  D3_events_ALL_OUT <- D3_events_ALL_OUT[name_event %in% diagnosis_concepts, ]
  cod_system_diagnosis <- unique(D3_events_ALL_OUT[!is.na(coding_system_of_code_first_event), coding_system_of_code_first_event])
  
  table_1cond <- data.table::data.table(a = c("Type of datasource", "Recommended end date", "Coding systems for diagnoses", "Attrition"),
                                 b = c("1", as.character(study_end), paste(cod_system_diagnosis, collapse = '/'), ""))
  setnames(table_1cond, "b", vect_recode_manufacturer[thisdatasource])
  

  table_1b <- rbind(table_1cond, table_1a, table_1b)
  table_1b <- table_1b[a == "Start population", a := "Persons in the instance of the data source, N"]
  table_1b <- table_1b[a == "A", a := "Sex or birth date missing or absurd, no dates of entry or exit, N"]
  table_1b <- table_1b[a == "D_death_before_study_entry", a := "Death before 1/1/2019, N"]
  table_1b <- table_1b[a == "E_no_observation_period_including_study_start", a := "Exit from the data source before 1/1/2019, N"]
  table_1b <- table_1b[a == "end population", a := "Persons in the data source at or after 1/1/2019, N"]

  setnames(table_1b, "a", "Condition")
  
  fwrite(table_1b, file = paste0(dummytables_april, "Table 1.csv"))
...
nameoutput <- paste0("Table 6, Cohort characteristics at start of study (1-1-2020)")
...
N_fup_pop <- D3_Vaccin_coh[, .(person_id, sex, date_vax2, type_vax_2, fup_vax2, age_at_date_vax_2)]
N_fup_pop <- N_fup_pop[!is.na(type_vax_2), ]
setnames(N_fup_pop, c("date_vax2", "type_vax_2", "fup_vax2","age_at_date_vax_2"),
...
nameoutput <- paste0("Table 11, COVID-19 vaccination by dose and time period between first and second dose (days)")
...
list_outcomes <- list_outcomes[str_detect(list_outcomes, "_narrow")]
...
for (outcome in list_recurrent_outcomes) {
  ...
  empty_concept <- data.table::data.table(year_event = event, Broad = character(1), Narrow = character(1))
  df_event <- data.table::data.table(year_event = character(), N = character(), Type = character())
  D3_events_ALL_OUT_30 <- copy(D3_events_ALL_OUT)[, date_event_30 := date_event + days(30)]
  setorderv(D3_events_ALL_OUT_30, c("unique_id", "date_event", "date_event_30"))
  
  if (is.na(splitted_outcome[2])) {
    df_temp <- copy(D3_events_ALL_OUT_30)[name_event == event, ]
    df_temp <- df_temp[, `:=`(row_id = rowid("person_id"))]
    df_temp <- df_temp[, `:=`(lag_end_date = fifelse(row_id > 1, shift(date_event_30), date_event_30))]
    df_temp <- df_temp[, `:=`(lag_end_date = as.integer(lag_end_date))]
    df_temp <- df_temp[, `:=`(lag_end_date = cummax(lag_end_date)), by = "unique_id"]
    df_temp <- df_temp[, `:=`(lag_end_date = as.Date(lag_end_date, "1970-01-01"))]
    df_temp <- df_temp[, `:=`(num_spell = fifelse(row_id > 1 & date_event <= lag_end_date, 0, 1))]
    df_temp[df_temp[,.I[which.min(date_event)], by = c("unique_id", "num_spell")][['V1']]]
    df_temp <- df_temp[, .N, by = "year_event"][, Type := "Narrow"]
    df_event <- data.table::rbindlist(list(df_event, df_temp))
  } else {
    for (type in c("narrow")) {
      concept <- paste0(event, "_", type)
      df_temp <- copy(D3_events_ALL_OUT_30)[name_event == concept, ]
      df_temp <- df_temp[, `:=`(row_id = rowid("person_id"))]
      df_temp <- df_temp[, `:=`(lag_end_date = fifelse(row_id > 1, shift(date_event_30), date_event_30))]
      df_temp <- df_temp[, `:=`(lag_end_date = as.integer(lag_end_date))]
      df_temp <- df_temp[, `:=`(lag_end_date = cummax(lag_end_date)), by = "unique_id"]
...
nameoutput <- paste0("Table 12, Number of incident cases entire study period")
